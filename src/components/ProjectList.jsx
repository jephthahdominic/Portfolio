...
const ProjectList = () => {
    ...
    return (
    <div className='w-full padding max-h-screen'>
        <div className='w-full flex justify-center max-sm:flex-col gap-10 mt-10 max-xl:mt-0'>
          {projects.map((project)=>(
            <div className='relative cursor-pointer' key={project.url} onMouseOver={()=>setDisplay(project.url)} onMouseOut={()=>setDisplay("")}> 
              <img src={project.img} alt="project snippet" className='xl:w-[700px] w-[300px] max-sm:w-full xl:h-[350px] h-[300px] brightness-60 object-cover'/>
              <div className={`absolute top-0 h-full w-full bg-black bg-opacity-50 flex flex-col justify-between opacity-100`}>
